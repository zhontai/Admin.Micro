...
public class PermissionService : BaseService, IPermissionService, IDynamicApi
{
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public async Task<PermissionGetGroupResponse> GetGroupAsync(long id)
    {
        var result = await _permissionRepository.GetAsync<PermissionGetGroupResponse>(id);
        return result;
    }
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public async Task<PermissionGetMenuResponse> GetMenuAsync(long id)
    {
        var result = await _permissionRepository.GetAsync<PermissionGetMenuResponse>(id);
        return result;
    }
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public async Task<PermissionGetApiResponse> GetApiAsync(long id)
    {
        var result = await _permissionRepository.GetAsync<PermissionGetApiResponse>(id);
        return result;
    }
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public async Task<PermissionGetDotResponse> GetDotAsync(long id)
    {
        var output = await _permissionRepository.Select
        .WhereDynamic(id)
        .ToOneAsync(a => new PermissionGetDotResponse
        {
    ...
    /// <returns></returns>
    public async Task<List<PermissionGetListResponse>> GetListAsync(string key, DateTime? start, DateTime? end)
    {
        if (end.HasValue)
        {
        ...
        var data = await _permissionRepository
            ...
            .ToListAsync(a=> new PermissionGetListResponse 
            {
    ...
    /// <returns></returns>
    [Obsolete("请使用PkgService.GetPkgPermissionListAsync查询套餐权限列表方法")]
    ...
    /// <returns></returns>
    public async Task<long> AddGroupAsync(PermissionAddGroupRequest request)
    {
        var entity = Mapper.Map<PermissionEntity>(request);
        entity.Type = PermissionType.Group;

        if (entity.Sort == 0)
        {
            var sort = await _permissionRepository.Select.Where(a => a.ParentId == request.ParentId).MaxAsync(a => a.Sort);
            entity.Sort = sort + 1;
        }
    ...
    /// <param name="request"></param>
    /// <returns></returns>
    public async Task<long> AddMenuAsync(PermissionAddMenuRequest request)
    {
        var entity = Mapper.Map<PermissionEntity>(request);
    ...
    /// <param name="request"></param>
    /// <returns></returns>
    public async Task<long> AddApiAsync(PermissionAddApiRequest request)
    {
        var entity = Mapper.Map<PermissionEntity>(request);
    ...
    /// <returns></returns>
    [AdminTransaction]
    public virtual async Task<long> AddDotAsync(PermissionAddDotRequest request)
    {
        var entity = Mapper.Map<PermissionEntity>(request);
        entity.Type = PermissionType.Dot;
        if (entity.Sort == 0)
        ...
        if (request.ApiIds != null && request.ApiIds.Any())
        {
            var permissionApis = request.ApiIds.Select(a => new PermissionApiEntity { PermissionId = entity.Id, ApiId = a }).ToList();
    ...
    /// <param name="request"></param>
    /// <returns></returns>
    public async Task UpdateGroupAsync(PermissionUpdateGroupRequest request)
    {
        var entity = await _permissionRepository.GetAsync(request.Id);
    ...
    /// <param name="request"></param>
    /// <returns></returns>
    public async Task UpdateMenuAsync(PermissionUpdateMenuRequest request)
    {
        var entity = await _permissionRepository.GetAsync(request.Id);
        entity = Mapper.Map(request, entity);
        await _permissionRepository.UpdateAsync(entity);
    ...
    /// <param name="request"></param>
    /// <returns></returns>
    public async Task UpdateApiAsync(PermissionUpdateApiRequest request)
    {
        var entity = await _permissionRepository.GetAsync(request.Id);
        entity = Mapper.Map(request, entity);
        await _permissionRepository.UpdateAsync(entity);
    ...
    /// <returns></returns>
    [AdminTransaction]
    public virtual async Task UpdateDotAsync(PermissionUpdateDotRequest request)
    {
        var entity = await _permissionRepository.GetAsync(request.Id);
        if (!(entity?.Id > 0))
        {
            throw Response.Exception("权限点不存在！");
        }
        ...
        if (request.ApiIds != null && request.ApiIds.Any())
        {
            var permissionApis = request.ApiIds.Select(a => new PermissionApiEntity { PermissionId = entity.Id, ApiId = a });
    ...
    /// <returns></returns>
    [AdminTransaction]
    [Obsolete("请使用PkgService.SetPkgPermissionsAsync设置套餐权限方法")]
    public virtual async Task SaveTenantPermissionsAsync(PermissionSaveTenantPermissionsRequest input)
    {
        //查询租户权限
