...
public class ViewService : BaseService, IViewService, IDynamicApi
{
    ...
    /// <param name="id"></param>
    /// <returns></returns>
    public async Task<ViewGetResponse> GetAsync(long id)
    {
        var result = await _viewRepository.GetAsync<ViewGetResponse>(id);
        return result;
    }
    ...
    /// <returns></returns>
    public async Task<List<ViewGetListResponse>> GetListAsync(string key)
    {
        var data = await _viewRepository
            .WhereIf(key.NotNull(), a => a.Path.Contains(key) || a.Label.Contains(key))
            .OrderBy(a => a.ParentId)
            .OrderBy(a => a.Sort)
            .ToListAsync<ViewGetListResponse>();
    ...
    /// <returns></returns>
    public async Task<long> AddAsync(ViewAddRequest input)
    {
    ...
    /// <param name="input"></param>
    /// <returns></returns>
    public async Task UpdateAsync(ViewUpdateRequest input)
    {
        var entity = await _viewRepository.GetAsync(input.Id);
        if (!(entity?.Id > 0))
        {
            throw Response.Exception("视图不存在！");
        }
    ...
    /// <returns></returns>
    [AdminTransaction]
    public virtual async Task SyncAsync(ViewSyncRequest input)
    {
        //查询所有视图
